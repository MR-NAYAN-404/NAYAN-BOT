module.exports = function({ api, models, Users, Threads, Currencies }) {
  ...
  return async function({ event }) {
    ...
    const { PREFIX, ADMINBOT, developermode, OPERATOR, APPROVED, approval, OWNER} = global.config;
    ...
    if (command && command.config) {
      ...
      const awto = 'awto'
      if (command.config.prefix === awto && !body.startsWith(PREFIX) && commandName.toLowerCase() !== command.config.name.toLowerCase()) {
        return;
      }
    }
    if (command && command.config) {
